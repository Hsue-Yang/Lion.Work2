    ...
    public class EntityUserConnect : EntitySys
    {
        ...
        public List<UserConnect> SelectUserConnectList(UserConnectPara para)
        {
            string commandText = string.Concat(new object[]
            {
                ...
                "WHERE dbo.FN_GET_SYSDATE(C.LAST_CONNECT_DT)+dbo.FN_GET_SYSTIME(C.LAST_CONNECT_DT)>={CONNECT_DT_BEGIN} ", Environment.NewLine,
                "  AND dbo.FN_GET_SYSDATE(C.LAST_CONNECT_DT)+dbo.FN_GET_SYSTIME(C.LAST_CONNECT_DT)<={CONNECT_DT_END} ", Environment.NewLine,
