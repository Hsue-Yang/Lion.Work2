    ...
    public class EntitySystemEDIPara : EntitySys
    {
        ...
        public List<SystemEDIPara> SelectSystemEDIParaList(SystemEDIParaPara para)
        {
            string commandText = string.Concat(new object[]
            {
                ...
                "FROM SYS_SYSTEM_EDI_PARA P INNER JOIN SYS_SYSTEM_EDI_JOB J ON P.EDI_JOB_ID=J.EDI_JOB_ID AND P.EDI_FLOW_ID= J.EDI_FLOW_ID ", Environment.NewLine,
                "JOIN SYS_SYSTEM_EDI_FLOW F ON J.EDI_FLOW_ID=F.EDI_FLOW_ID ", Environment.NewLine,
                "JOIN SYS_SYSTEM_MAIN M ON F.SYS_ID=M.SYS_ID ", Environment.NewLine,
                "WHERE M.SYS_ID={SYS_ID} AND F.EDI_FLOW_ID={EDI_FLOW_ID} AND J.EDI_JOB_ID={EDI_JOB_ID} ", Environment.NewLine,
                "ORDER BY M.SYS_ID, F.EDI_FLOW_ID, P.SORT_ORDER ", Environment.NewLine
            });
