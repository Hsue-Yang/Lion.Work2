...
namespace LionTech.Entity.B2P.Sys
{
    public class EntitySystemFunAssign : EntitySys
    {
        ...
        public SystemFun SelectSystemFunInfor(SystemFunPara para)
        {
            string commandText = string.Concat(new object[]
            {
                "SELECT dbo.FN_GET_NMID(FUN_ACTION_NAME, {FUN_NM}) AS FUN_NM ", Environment.NewLine,
                "FROM SYS_SYSTEM_FUN ", Environment.NewLine,
                "WHERE SYS_ID={SYS_ID} ", Environment.NewLine,
                "  AND FUN_CONTROLLER_ID={FUN_CONTROLLER_ID} ", Environment.NewLine,
                "  AND FUN_ACTION_NAME={FUN_ACTION_NAME} ", Environment.NewLine,
        ...
        public List<SystemFunAssign> SelectSystemFunAssignList(SystemFunAssignPara para)
        {
            string commandText = string.Concat(new object[]
            {
                "SELECT U.USER_ID, R.USER_NM AS USER_NM ", Environment.NewLine,
                "FROM SYS_USER_FUN U ", Environment.NewLine,
                "JOIN RAW_CM_USER R ON U.USER_ID=R.USER_ID ", Environment.NewLine,
                "WHERE U.SYS_ID={SYS_ID} AND U.FUN_CONTROLLER_ID={FUN_CONTROLLER_ID} AND U.FUN_ACTION_NAME={FUN_ACTION_NAME} ", Environment.NewLine,
                "  AND U.IS_ASSIGN='Y' ", Environment.NewLine,
                "ORDER BY U.USER_ID; "
            });
            ...
            if (dataTable.Rows.Count > 0)
            {
                ...
                foreach (DataRow dataRow in dataTable.Rows)
                {
                    SystemFunAssign systemFunAssign = new SystemFunAssign()
                    {
                        UserID = new DBVarChar(dataRow[SystemFunAssign.DataField.USER_ID.ToString()]),
                        UserNM = new DBNVarChar(dataRow[SystemFunAssign.DataField.USER_NM.ToString()])
                    };
                    systemFunAssignList.Add(systemFunAssign);
                }
                return systemFunAssignList;
            }
            return null;
        }
