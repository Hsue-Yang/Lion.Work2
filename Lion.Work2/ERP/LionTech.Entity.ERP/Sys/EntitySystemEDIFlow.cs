...
namespace LionTech.Entity.ERP.Sys
{
    public class EntitySystemEDIFlow : EntitySys
    {
        ...
        public class SystemEDIFlowPara : DBCulture
        {
            ...
            {
                SYS_ID, SYS_NM, EDI_FLOW, SCH_FREQUENCY, UPD_USER_ID, CULTURE_ID
            }
        ...
        public List<SystemEDIFlow> SelectSystemEDIFlowList(SystemEDIFlowPara para)
        {
            ...
            if(!string.IsNullOrWhiteSpace(para.SCHFrequency.GetValue()))
            {
                commandWhere = string.Concat(new object[]{
                "  AND E.SCH_FREQUENCY={SCH_FREQUENCY} ", Environment.NewLine
                });
            }
            string commandText = string.Concat(new object[]
            {
                ...
                "     , dbo.FN_GET_NMID(E.SCH_FREQUENCY, dbo.FN_GET_CM_NM('0005', E.SCH_FREQUENCY, {CULTURE_ID})) AS SCH_FREQUENCY ", Environment.NewLine,
            ...
            dbParameters.Add(new DBParameter { Name = SystemEDIFlowPara.ParaField.SCH_FREQUENCY.ToString(), Value = para.SCHFrequency });
            dbParameters.Add(new DBParameter { Name = SystemEDIFlowPara.ParaField.CULTURE_ID.ToString(), Value = new DBVarChar(para.CultureID) });
            ...
            if (dataTable.Rows.Count > 0)
            {
                ...
                {
                    SystemEDIFlow ediFlow = new SystemEDIFlow()
                    {
                        ...
                        SCHFrequency = new DBVarChar(dataRow[SystemEDIFlow.DataField.SCH_FREQUENCY.ToString()]),
        ...
        public class SystemEDIIPAddressPara
        {
            public enum ParaField
            {
                SYS_ID
        ...
        public DBVarChar SelectSystemEDIIPAddress(SystemEDIIPAddressPara para)
        {
            string commandText = string.Join(Environment.NewLine, new object[]
            {
                "SELECT IP_ADDRESS ",
                "FROM SYS_SYSTEM_IP ",
                "WHERE SUB_SYS_ID = {SYS_ID} ",
                "AND IS_FILE_SERVER = 'Y'"
            });

            List<DBParameter> dbParameters = new List<DBParameter>();
            dbParameters.Add(new DBParameter { Name = SystemEDIIPAddressPara.ParaField.SYS_ID.ToString(), Value = para.SysID });
            return new DBVarChar(ExecuteScalar(commandText, dbParameters));
        }
    }
}