...
function SystemFunDetailForm_onLoad(formElement) {
    ...
    $('#SubSysID, #PurviewID, #FunControllerID, #FunType', _formElement).combobox({
        width: 180,
        isRemoveButton: true
    });

    $('select[id$=FunMenuSysID]', _formElement).combobox({
        width: 180,
        isRemoveButton: true,
        onChange: FunMenuSysID_onChange
    });

    $('select[id$=FunMenu]', _formElement).combobox({
        width: 180,
        isRemoveButton: false
    });

    $('select[id$=FunMenuXAxis], select[id$=FunMenuYAxis]', _formElement).combobox({
        isRemoveButton: false
    });
...
function FunMenuSysID_onChange(event) {
    var funMenu = event.select.context.id.split('.')[0] + '.FunMenu';
    var objFunMenu = $("select[name='" + funMenu + "']");
    objFunMenu.find('option').remove();
    objFunMenu.combobox('Selected').input.val('');
...
function CheckFields() {
    $('input[inputtype^=TextBox]').each(function () {
